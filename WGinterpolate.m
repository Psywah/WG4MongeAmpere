...
for p = 1:2
    ...
    for i = 1:2
        u(Nn+NEn + ibn*4 +(i-1)*2+p) =  oldu(elem(:,1)).* Dphip(:,i,1) + ...
            oldu(elem(:,2)).* Dphip(:,i,2) + ...
            oldu(elem(:,3)).* Dphip(:,i,3) + ...
             oldu(N+elem2edge(:,1)).*Dphip(:,i,4) + ...
             oldu(N+elem2edge(:,2)).*Dphip(:,i,5) + ...
             oldu(N+elem2edge(:,3)).*Dphip(:,i,6);
    end
end
